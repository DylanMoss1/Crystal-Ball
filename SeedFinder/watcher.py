#!/usr/bin/env python3
"""Seed Finder watcher - bridges the Balatro mod to the Immolate searcher.
...
def run_immolate(binary, filt, query, timeout):
    ...
    cmd = [binary, "-f", filt, "--first", "-q", "-j", query]
    try:
...
def main():
    ap = argparse.ArgumentParser(description="Seed Finder <-> Immolate bridge")
    ap.add_argument(
        "--dir", required=True, help="SeedFinder handshake dir (in the LOVE save dir)"
    )
    ...
    last_id = None
    print(f"[SeedFinder] watching {req_path}\n[SeedFinder] immolate: {args.immolate}")
    ...
    while True:
        ...
        print(f"[SeedFinder] {rid} -> {payload}")
        time.sleep(args.interval)
