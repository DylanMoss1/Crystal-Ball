...
//   - a shop item slot (incl. reroll slots; see SHOP_BASE_SLOTS),
...
// Shop card slots per view. Ante N is scanned as the base shop plus (N-1)
// rerolls, i.e. N views => SHOP_BASE_SLOTS * N item draws (ante 1 = no rerolls).
#define SHOP_BASE_SLOTS 2
...
long filter(instance* inst, __global int* q, int qlen) {
    ...
    // Forward pass A: shop item slots. Ante N is the base shop + (N-1) rerolls,
    // so it draws SHOP_BASE_SLOTS * N items (ante 1 has no rerolls). Reading the
    // queue forward this way mirrors the player rerolling through ante N's shop.
    for (int ante = 1; ante <= maxAnte; ante++) {
        bool covered; ANTE_COVERED(ante, covered);
        if (!covered) continue;
        int shopDraws = SHOP_BASE_SLOTS * ante;
    ...
    // Forward pass B: shop booster packs per ante (analyzer convention: 4 in
    // ante 1, 6 after). Packs can't be rerolled, so this count is fixed.
